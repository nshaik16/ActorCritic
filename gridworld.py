...
if __name__ == '__main__':
    ...
    import valueIterationAgents, qlearningAgents, reinforceAgent, actorCriticAgents
    a = None
    if opts.agent == 'value':
    ...
    elif opts.agent == "actorcritic":
        gridWorldEnv = GridworldEnvironment(mdp)
        actionFn = lambda state: mdp.getPossibleActions(state)
        learnOpts = {'gamma': opts.discount,
                      'actionFn': actionFn},
                      'alpha_theta': opts.learningRate, # Actor learning rate
                      'alpha_w': opts.learningRate, # Critic learning rate
        a = actorCriticAgents.actorCriticAgent(**learnOpts)
    elif opts.agent == 'q':
